...
const DniForm = ({ dni, onSave = () => {}, onCancel = () => {} }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const url = `http://localhost:8080/api/tipos-documento${dni && dni.id ? `/${dni.id}` : ''}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Error en la respuesta del servidor: ${errorData.message || response.statusText}`);
      }
